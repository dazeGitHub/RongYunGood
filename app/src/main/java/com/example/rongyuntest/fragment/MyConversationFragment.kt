            ...
            if (RongConfigCenter.conversationConfig()
                ...
                if (count != null && count > 0) {
                    ...
                    mUnreadHistoryMessageNum?.text = MessageFormat.format(
                        this@MyConversationFragment.getString(R.string.rc_unread_message),
                ...
                if (count != null && count > 0) {
                    ...
                    mUnreadMentionMessageNum?.text =
                        this@MyConversationFragment.getString(
                            R.string.rc_mention_messages, arrayOf<Any>(
                                "($count)"
                            )
                        )
                } else {
...
//        private set
    ...
    override fun onRequestPermissionsResult(
        ...
        if (PermissionCheckUtil.checkPermissionResultIncompatible(permissions, grantResults)) {
            if (this.context != null) {
                ToastUtils.s(this.context, this.getString(R.string.rc_permission_request_failed))
            }
    ...
    override fun onCreateView(
        ...
        mList = rootView.findViewById<View>(R.id.my_rc_message_list) as RecyclerView
    ...
    override fun onDestroyView() {
        kotlin.runCatching {
            ...
            RLog.e("TAG", "onDestroyView() errMsg = " + it.message)
        }
    ...
    override fun onWarningDialog(msg: String?) {
        ...
        if (window != null) {
            ...
            val tv = window.findViewById<View>(R.id.my_rc_cs_msg) as TextView
            tv.text = msg
            window.findViewById<View>(R.id.my_rc_btn_ok).setOnClickListener {
                alertDialog.dismiss()
    ...
    private fun showLoadMessageDialog(callback: GetMessageCallback?, list: List<Message>) {
        AlertDialog.Builder(this.activity, 5)
            .setMessage(this.getString(R.string.rc_load_local_message)).setPositiveButton(
                this.getString(R.string.rc_dialog_ok)
            ) { dialog, which -> callback?.onSuccess(list, true) }
            .setNegativeButton(
                this.getString(R.string.rc_cancel)
